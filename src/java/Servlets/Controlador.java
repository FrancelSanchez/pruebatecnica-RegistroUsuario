...
public class Controlador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(accion.equals("add")){ // CUANDO EL USUARIO PULSA EL BOTON DE AGREGAR USUARIO EN EL INDEX ESTE REALIZA ESTA VALIDACION PARA ENVIARLO A LA PAGINA DE AGREGAR USUARIO
            ...
            String nom = request.getParameter("textnom");// OBTENIENDO EL STRING DEL CAMPO NOMBRE EN LA PAGINA
            String ap = request.getParameter("textap"); // OBTENIENDO EL STRING DEL CAMPO APELLIDO EN LA PAGINA
            String desc = request.getParameter("textcomentario"); // OBTENIENDO EL STRING DEL CAMPO COMENTARIO EN LA PAGINA
            Usuario usuario = new Usuario(); // INSTANCIANDO EL OBJETO USUARIO
            usuario.setNombre(nom); // SETEANDO EL NOMBRE DEL USUARIO
            usuario.setApellido(ap); // SETEANDO EL APPELIDO DEL OBJETO USUARIO
            usuario.setComentario(desc); // SETEANDO EL COMENTARIO
           
            UsuarioService userservice = new UsuarioService();     // INSTNACIANDO EL OBJETO DEL WEB SERVICE   
            userservice.crearUsuarios(usuario); // EJECUANDO EL METODO CREAR USUARIO DENTRO DEL WEB SERVICE
            acceso = index;
        }
